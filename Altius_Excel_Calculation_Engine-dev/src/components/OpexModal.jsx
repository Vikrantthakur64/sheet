 ...
 *   tenure      string
 *   opexResult  {
 *     rows:         { leaseRent, energyExpense, rAndM, admin, salary, totalOpex }[]
 *     rates:        { leaseRentPerSite, energyChargesPerTenant, rAndMPerSite }
 *     onRollSalary: number[]
 *     offRollSalary: number[]
 *   }
 */
export default function OpexModal({ open, onClose, yearLabels, tenure, opexResult }) {
  ...
  if (!open || !opexResult) return null

  const { rows, onRollSalary, offRollSalary } = opexResult
  ...
  const tdLblSub  = `border border-slate-200 px-3 py-2 text-xs font-normal text-slate-400 italic bg-slate-50 whitespace-nowrap sticky left-0 z-10`
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 bg-[#0d2045] rounded-t-2xl">
          <div>
            <h2 className="text-base font-bold text-white tracking-wide">Opex Projection</h2>
            <p className="text-blue-300 text-xs mt-0.5">
              Operating Expenditure · {tenure} year projection
            </p>
          </div>
          <button
            onClick={onClose}
            ...
            aria-label="Close opex projection"
          >
        ...
        <div className="flex-1 px-4 md:px-6 py-5 space-y-6 overflow-y-auto">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            {[
              { label: 'Lease Rent (Yr 1)',  value: fmt(yr1.leaseRent),   color: 'bg-sky-50     border-sky-200     text-sky-700'     },
              { label: 'Admin Cost (Yr 1)',  value: fmt(yr1.admin),       color: 'bg-blue-50    border-blue-200    text-blue-700'    },
              { label: 'Total Opex (Yr 1)',  value: fmt(yr1.totalOpex),   color: 'bg-indigo-50  border-indigo-200  text-indigo-700'  },
            ].map((card) => {
          ...
          <SectionTable title="Operating Expenditure — Year-by-Year (INR MM)">
            ...
            <tr className="bg-white hover:bg-slate-50">
              <td className={tdLbl}>Lease Rent</td>
              {rows.map((r, i) => <td key={i} className={tdNum}>{fmt(r.leaseRent)}</td>)}
            </tr>
            <tr className="bg-white hover:bg-slate-50">
              <td className={tdLbl}>Energy Expense</td>
              {rows.map((r, i) => <td key={i} className={tdNum}>{fmt(r.energyExpense)}</td>)}
            </tr>
            <tr className="bg-white hover:bg-slate-50">
              <td className={tdLbl}>R&amp;M</td>
              {rows.map((r, i) => <td key={i} className={tdNum}>{fmt(r.rAndM)}</td>)}
            </tr>

            <SepRow />

            <tr className="bg-white hover:bg-slate-50">
              <td className={tdLbl}>Admin Cost</td>
              {rows.map((r, i) => <td key={i} className={tdNum}>{fmt(r.admin)}</td>)}
            </tr>

            <SepRow />

            <tr className="bg-white hover:bg-slate-50">
              <td className={tdLblSub}>↳ On Roll</td>
              {onRollSalary.map((v, i) => <td key={i} className={`${tdNum} text-slate-500`}>{fmt(v)}</td>)}
            </tr>
            <tr className="bg-white hover:bg-slate-50">
              <td className={tdLblSub}>↳ Off Roll</td>
              {offRollSalary.map((v, i) => <td key={i} className={`${tdNum} text-slate-500`}>{fmt(v)}</td>)}
            </tr>
            <tr className="bg-white hover:bg-slate-50">
              <td className={tdLbl}>Total Salary</td>
              {rows.map((r, i) => <td key={i} className={tdNum}>{fmt(r.salary)}</td>)}
            </tr>
            ...
            <tr>
              ...
              {rows.map((r, i) => <td key={i} className={tdNumBold}>{fmt(r.totalOpex)}</td>)}
            </tr>
          </SectionTable>
