...
export default function DebtINVITModal({ open, onClose, yearLabels, tenure, result }) {
  ...
  const { capexDebt, spvDebt } = result
    ...
    return (
      <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
        <div className="bg-[#b8d4ea] px-4 py-2 border-b border-slate-300 flex items-center justify-between">
          ...
          <span className="text-[10px] text-slate-500">Year 1 Closing: {fmt(yr1.closing)} INR MM</span>
        </div>
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label="Debt INVIT Projection"
    >
      <div
        ref={panelRef}
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 bg-[#0d2045] rounded-t-2xl">
          <div>
            <h2 className="text-base font-bold text-white tracking-wide">Debt — INVIT Projection</h2>
            <p className="text-blue-300 text-xs mt-0.5">
              INVIT-Level Debt Schedule · {tenure} year projection
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 px-4 md:px-6 py-5 space-y-6 overflow-y-auto">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            {[
              { label: 'Capex Debt Opening (Yr 1)', value: fmt(yr1c.opening),     color: 'bg-blue-50   border-blue-200   text-blue-700'   },
              { label: 'Capex Debt Closing (Yr 1)', value: fmt(yr1c.closing),     color: 'bg-indigo-50 border-indigo-200 text-indigo-700' },
              { label: 'Finance Cost (Yr 1)',        value: fmt(yr1c.financeCost), color: 'bg-orange-50 border-orange-200 text-orange-700' },
            ].map((card) => {
          ...
          {spvDebt && (
            <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
              <div className="bg-[#b8d4ea] px-4 py-2 border-b border-slate-300">
                <span className="text-xs font-bold text-[#1e3a5f] uppercase tracking-wide">SPV Refinancing Debt — Year-by-Year (INR MM)</span>
              </div>
              <div className="overflow-x-auto">
                <table className="border-collapse text-sm" style={{ tableLayout: 'auto', minWidth: '100%' }}>
                  <thead>
                    <tr>
                      <th className={`${thCls} text-left sticky left-0 z-20 bg-slate-50`} style={{ minWidth: 200 }}>Line Item</th>
                  ...
                  <tbody>
                    <tr className="bg-white hover:bg-slate-50">
                      <td className={tdLbl}>Opening</td>
                      {spvDebt.map((r, i) => <td key={i} className={tdNum}>{fmt(r.opening)}</td>)}
                    </tr>
                    <tr className="bg-white hover:bg-slate-50">
                      <td className={tdLbl}>Addition</td>
                      {spvDebt.map((r, i) => <td key={i} className={tdNum}>{fmt(r.additions)}</td>)}
                    </tr>
                    <tr className="bg-white hover:bg-slate-50">
                      <td className={tdLbl}>Repayment</td>
                      {spvDebt.map((r, i) => <td key={i} className={tdNum}>{fmt(r.repayments)}</td>)}
                    </tr>
                    <tr>
                      <td className={tdLblBold}>Closing</td>
                      {spvDebt.map((r, i) => <td key={i} className={tdNumBold}>{fmt(r.closing)}</td>)}
                    </tr>
                    <tr className="bg-white hover:bg-slate-50">
                      <td className={tdLbl}>Repayment Schedule</td>
                      {spvDebt.map((r, i) => <td key={i} className={tdNum}>{fmtPct(r.repayPct)}</td>)}
                    </tr>
                    <tr>
                      <td className={tdLblBold}>Interest Expense</td>
                      {spvDebt.map((r, i) => <td key={i} className={tdNumBold}>{fmt(r.financeCost)}</td>)}
                    </tr>
                  </tbody>
                </table>
              </div>
          ...
          <ScheduleTable title="Capex Debt Schedule — Year-by-Year (INR MM)" schedule={capexDebt} />
